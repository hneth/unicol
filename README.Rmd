...
# unicol `r packageVersion('unicol')` <img src = "man/figures/logo.png" align = "right" alt = "unicol::" width = "160px" alt="" /> 
...
(See the vignette on **[Color palettes](https://hneth.github.io/unicol/articles/color_pals.html)** for an overview of included color palettes and institutions.) 
...
set.seed(127) # reproducible randomness
...
```{r unicol-pals-example-1-figure, echo = FALSE, eval = TRUE, dpi = 100, fig.width  = 7.5, out.width = "600px", fig.asp = 2.0, fig.cap = fig_caption}
...
```{r example-similar-sets, echo = FALSE, eval = FALSE}
bg_col <- "grey85"

# Red and grey:
seecol(list(bu, harvard_1, cornell_1, 
            lancaster_1, yorku_1, groningen_1, 
            mit, uchicago_1, 
            mun_1, CMU_1_core, rpi_1, cardiff_1, stanford_1), 
       main = "Red and grey university colors", col_bg = bg_col)

# Blues:
seecol(list(duke_1, jhu_1, monash_1, uni_goettingen_1, hu_2, uni_mannheim_1, 
            # uni_koeln_1, uni_hamburg_1, 
            auckland_1, 
            uni_stuttgart_1, uni_konstanz_1,
            ubc, trinity_1),
       main = "Mostly blue and white university colors", col_bg = bg_col)

# Greens:
seecol(list(ohio_uni_1, limerick_1, stirling_1, lmu_1, fu_1), 
       main = "Mostly green university colors", col_bg = bg_col)


# Yellow and blue:
seecol(list(UCLA_1, uci_1, pitt_1), 
       main = "Yellow and blue university colors", col_bg = bg_col)


# Similar pairs:
seecol(list(groningen_1, yorku_1), main = "Spot the difference: ", col_bg = bg_col)
seecol(list(notredame_1, uni_jena_1), main = "Spot the difference: (2)", col_bg = bg_col)
```
