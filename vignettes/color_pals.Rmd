...
# Look up URLs and create links for all_pals:
pal_urls <- lookup(all_pals, unicol_data, v_1 = "pal", v_2 = "url")
pal_inst_links <- link_inst(all_pals)
pal_doc_links  <- link_github_doc(all_pals)
pal_lengths    <- sapply(X = unicol_data$pal, FUN = n_col)

# mean(pal_lengths)  # mean number of colors 
...
# Create summary df:
pal_df <- data.frame(inst = pal_inst_links, 
                     # url = pal_urls, 
                     country = pal_countries, 
                     pal = pal_doc_links,
                     n_col = pal_lengths)

row.names(pal_df) <- 1:nrow(pal_df)
...
knitr::kable(pal_df, caption = df_caption, col.names = col_names, row.names = TRUE)
```
