...
use the `usecol()` function (of the **[unikn](`r `url_unikn_cran`)** package) in combination with any color palette (e.g., `pal_unikn`) as the `col`\ argument: 
...
1. Directly using a color palette:
...
my_col <- unikn::usecol(mit_pal, n = 7, alpha = .50)  # with transparency
