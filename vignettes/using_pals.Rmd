...
The **unicol** color palettes are typically provided as named vectors of color objects. Thus, when creating **base** R visualizations (e.g., using `plot()` for creating a scatterplot), we can directly use a color palette as the `col`\ argument. 
Alternatively, we can first modify a color palette (e.g., change its number of colors or add transparency) with the `usecol()` function of the **[unikn](`r `url_unikn_cran`)** package and then use the modified palette as the `col`\ argument of an R graphics function. 
...
my_col <- unikn::usecol(berkeley_1, n = 7, alpha = .67)  # with transparency
...
```{r seecol-example-1, eval = FALSE, fig.width = 5, fig.asp = .80, fig.align = 'center'}
...
```{r set-seed-HCL-pals, eval = FALSE, echo = FALSE}
set.seed(4 * pi)  # reproducible randomenss 
```

- Generate random HCL\ palettes (and save them as a list):

```{r seecol-example-2a, eval = FALSE, fig.width = 6, fig.asp = 1.0, fig.align = 'center'}
# Parameters:
...
```{r seecol-example-2b, eval = FALSE, fig.width = 6, fig.asp = 1.0, fig.align = 'center'}
seecol(pal = l_pal, 
       pal_names = s_pal,
...
```{r simcol-example-01, eval = FALSE, fig.width = 5, fig.asp = .90, fig.align = 'center', fig.show = "hold"}
simcol("forestgreen")
```
...
```{r simcol-example-02, eval = FALSE, fig.width = 5, fig.asp = .90, fig.align = 'center', fig.show = "hold"}
...
```{r simcol-example-03, eval = FALSE, fig.width = 5, fig.asp = .90, fig.align = 'center', fig.show = "hold"}
...
```{r newpal-example, eval = FALSE, fig.width = 5, fig.asp = .9, fig.align = 'center'}
# 1. Choose colors:
...
```{r grepal-example-1, eval = FALSE, fig.width = 5, fig.height = 4, fig.align = 'center'}
...
```{r grepal-example-2, eval = FALSE, fig.width = 6, fig.asp = .50, fig.align = 'center'}
...
```{r grepal-example-3, eval = FALSE, fig.width = 6, fig.align = .50, fig.align = 'center'} 
...
```{r shades-of-example-1, eval = FALSE, fig.width = 5, fig.asp = .90, fig.align = 'center'}
greys <- shades_of(10)
...
```{r shades-of-example-3, eval = FALSE, fig.width = 5, fig.asp = .90, fig.align = 'center'}
...
```{r gradient-example-1, eval = FALSE, fig.width = 5, fig.asp = .90, fig.align = 'center'}
